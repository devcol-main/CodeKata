| Solved Date: 2026-06-19-Fri | [Problem Link](https://school.programmers.co.kr/learn/courses/30/lessons/161990?language=cpp)
...
vector<int> solution(vector<string> wallpaper) {
    ...
    // 최소값은 가장 큰 값으로, 최대값은 가장 작은 값으로 초기화합니다.
    int lux, luy,rdx, rdy;

    int row_size = wallpaper.size();
    int col_size = wallpaper[0].size();

    for (int r = 0; r < row_size; ++r) 
    {
        for (int c = 0; c < col_size; ++c) 
        {            
            // 파일('#')을 발견했을 때 좌표를 갱신합니다.
            if (wallpaper[r][c] == '#') 
            {
                lux = min(lux, r);
                luy = min(luy, c);
    ...
    // 격자점 기준으로 끝점은 파일이 있는 칸의 다음 칸(오른쪽 아래 꼭짓점)이어야 하므로 +1을 해줍니다.
